...
- Рабочая область (`layoutWorkArea()`, `MONITORINFO.rcWork`) — отдельная вещь
  ...
  строкой `warn`. Плитку по уже разрешённым зонам это не касается вовсе: она
  идёт дальше без рабочей области, и предупреждение о ненайденном по зоне
  мониторе в этом случае не пишется — иначе оно врало бы про «считаю по
  главному», хотя плитка ляжет ровно в зоны и никакого отката не будет.
