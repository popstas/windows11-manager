...
describe('cursorRule', () => {
  ...
  it('просьба «не расставлять» перекладывается из точки в правило', () => {
    // Признак приезжает внутри точки (`parseCursorPoint`), а решает по нему
    // `placeByCursor`. Потеряйся он здесь — окно встало бы под курсором и
    // через секунду уехало в слот, где эта сессия жила вчера.
    const pinned = (cursor) => cursorRule({
      win: win({ x: 0, y: 0, width: 1000, height: 800 }),
      cursor,
      slot: null,
      monitorAt: () => MON,
    }).pinned;
    expect(pinned({ x: 100, y: 100, noAutoplace: true })).toBe(true);
    expect(pinned({ x: 100, y: 100, noAutoplace: false })).toBe(false);
    // Пикер прежней версии поля не шлёт вовсе: это «расставляй как обычно».
    expect(pinned({ x: 100, y: 100 })).toBe(false);
  });
