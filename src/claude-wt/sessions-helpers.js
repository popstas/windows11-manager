...
/**
 * Свежесть по тому, что известно про сессию вообще: отметка хука либо возраст
 * транскрипта, что новее. Общая шкала для записи с отметкой и записи без неё.
 */
function freshnessOf(s, probe) {
  return Math.max(stampOf(s, probe), s?.mtime ?? 0);
}
 ...
 *
 * - у обеих есть отметка — она и решает, ради этого всё и заводилось;
 * - у обеих нет — прежнее правило (`live`, потом mtime); хук не установлен, и
 *   спорить не о чем;
 * - у одной есть, у другой нет — сравнивать ноль с чужой отметкой как число
 *   нельзя. Ноль значит «хук про эту сессию не писал», а не «активности не
 *   было с 1970 года», и запись без отметки проигрывала по одному факту её
 *   отсутствия. Замерено 2026-08-12: две живые сессии
 *   `obsidian-agent-workspace`, у рабочей `activityAt: 0` (хук ещё не
 *   срабатывал), у суточной тёзки — позавчерашняя отметка. Окно ушло к
 *   старой, у выбранной в пикере сессии окна «не было», и вместо подъёма
 *   открывалось второе окно — каждый раз заново. Сравниваем то, что есть у
 *   обеих: свежесть транскрипта.
 */
function byActivityThen(probe) {
  return (a, b) => {
    const sa = stampOf(a, probe);
    const sb = stampOf(b, probe);
    const diff = sa && sb
      ? sb - sa
      : (!sa && !sb ? 0 : freshnessOf(b, probe) - freshnessOf(a, probe));
