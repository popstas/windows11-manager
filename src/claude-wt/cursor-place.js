...
function cursorRule({ win, cursor, slot, monitorAt = getMonitorByPoint }) {
  ...
  // по нему `placeByCursor`, одна на все дороги постановки по курсору.
  //
  // `remembered` — у окна было своё место, и мы его только что перебили.
  // Спрашивается ровно затем же, зачем `pinned`: без пометки демон при первой
  // же привязке вернёт окно в запомненные границы и на запомненный стол
  // (`step` в `tracker-helpers.js`), то есть отменит просьбу человека — и
  // отменит молча, через пару тиков после того, как окно уже встало верно.
  return { rule, primary, pinned: cursor?.noAutoplace === true, remembered: Boolean(slot?.bounds) };
}

/**
 * Поставить окно по правилу и, если просили, закрыть его от автоматики.
 *
 * Поводов три. Неглавный экран — так просил человек, и оговорка не лишняя: на
 * главном экране правила из `config.windows` делают ровно то, чего от них
 * ждут, а выключенная там расстановка выглядела бы поломкой конфига. Прямая
 * просьба пикера (`noAutoplace` в теле, Ctrl на строке списка) — та экран не
 * спрашивает вовсе: галка «на активном экране» отвечает на вопрос «как
 * открывать всегда», а модификатор — «как открыть вот эту», и на главном
 * экране он значит ровно то же, что на соседнем.
 *
 * И перебитая память о месте (`remembered`) — оговорка про главный экран сюда
 * не распространяется, и это исправление. Она написана про окна, у которых
 * своего места ещё нет; у окна со слотом спор другой и решается не в нашу
 * пользу: демон при первой привязке тащит такое окно в запомненные границы и
 * на запомненный стол. Без пометки галка «на активном экране» работала бы
 * ровно до этого тика — то есть отменяла бы сама себя, и чаще всего, потому
 * что главный экран у человека и есть обычный.
 *
...
async function placeByCursor(target, place, what) {
  ...
  if (!target.primary || target.pinned || target.remembered) markNoAutoplace(target.rule.window);
  return true;
}
