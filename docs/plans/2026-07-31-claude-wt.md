...
## Открытые вопросы, унаследованные задачами 6 и 9

Выявлены при исполнении задач 1-5, 7, 8 и финальном ревью. Ни один не блокирует
уже сделанный чистый слой, но каждый требует решения при написании демона.

- **Троттл предупреждений в `sessions.js` не закреплён тестом.** Мутант, убирающий
  проверку интервала, выживает. Демон тикает раз в секунду по сетевому пути
  (`V:`), так что сломанный троттл превращается в поток строк в логе.
- **Виртуальный стол восстанавливается только попутно с переносом.** Поле
  `desktop` едет на `actions`, а `action` подавляется, когда координаты уже
  совпадают. Окно на верном месте, но на чужом столе, назад не вернётся. Демону
  либо сравнивать столы самому, либо подавать текущий стол окна в `step()`.
- **Успешный зажатый перенос всё равно запишет зажатую позицию** следующим тиком.
  Чтобы сохранить исходные координаты после отключения монитора, нужен признак
  «позиция смещена клампом, а не выбрана пользователем» — такой концепции в
  дизайне нет.
- **Сопоставление заголовков точное.** Если Windows Terminal начнёт добавлять к
  заголовку префикс или украшение, слот не создастся и об этом ничего не будет
  сказано. Нужна диагностика «видел окно терминала, но не смог сопоставить».
- **Дедупликация записи состояния не сработает как написана:** `updated`
  меняется каждый тик, поэтому сравнение сериализованного состояния всегда даёт
  различие. Сравнивать `slots` и `lastLayout`, а не всю структуру.
