...
При каждом открытии пикера (`windows/claude-sessions-start`) windows-mqtt делает fire-and-forget `ssh … ccfzf --dump`, чтобы обновить `V:/.ccfzf.sessions.json`. Без этого свежая или переименованная сессия может минутами не попасть в индекс.
...
| `usage` залипает на чужих строках после смены порядка | Пустой/пробельный текст openHASP не стирает; HA ещё обрезает пробел у шаблона. В `slotUsage` всегда непустое: цифры или возраст (`5m`), иначе `-` |
