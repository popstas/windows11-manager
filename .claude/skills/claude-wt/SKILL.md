...
**Project hotkeys.** Список живёт в `claudeWt.projects` здесь, в `windows11-manager.config.js`, и второго конфига больше нет — ключ `projects:` из конфига пикера удалён. Демон кладёт список в файл трекера полем `projects` — только записи с непустым хоткеем, `profile` не едет намеренно: это знание про Windows Terminal на этой машине, читателю бесполезное. `ccfzf --state` клеит `hotkey` к строкам `projects[]` по пути (`merge_project_hotkeys`) и заводит строку проекту, у которого нет ни сессий, ни закладки. Регистрирует клавиши сам ccfzf-picker (`project_hotkeys.rs`), и только на машине, где `windowHost` совпал с конфигом пикера. Если Ctrl+F11/F12 пропали — смотреть не stderr (у приложения из трея его никто не читает), а статуслайн пикера: занятую комбинацию он называет сам.
...
**Второй издатель чаще свой, чем чужой.** Строки на плате, которые «постоянно
меняют порядок», — это почти всегда несколько процессов, публикующих
`…/claude/slot/N` каждый по своему таймеру. Источником считался windows-mqtt
(см. выше), но 2026-08-09 их оказалось три, и все три — свои: `taskkill /IM
windows11-manager-tray.exe /F` убивает трей, не дав ему погасить детей, и
`node src/index.js mqtt` от каждого прошлого запуска остаётся жить сиротой.
Внешне это неотличимо от сломанной сортировки: сирота работает на коде своего
времени, со своими настройками, и раз в пятнадцать секунд перекладывает слоты
по-своему. Проверять первым делом — `Get-CimInstance Win32_Process` с фильтром
по `index.js (mqtt|claude-wt)`: живых должно быть по одному, и `ppid` у обоих —
pid текущего трея. `data/scripts/deploy-pc.sh` теперь убирает их сам, но трей,
убитый мимо деплоя, сирот снова оставит: у самого приложения такой уборки нет.
